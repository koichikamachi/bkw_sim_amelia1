...
def _setup_additional_investments_internal(num_investments: int, exit_year: int) -> List[AdditionalInvestmentParams]:
    ...
    for i in range(1, num_investments + 1):
        with st.sidebar.expander(f"第{i}回 追加投資", expanded=True):
            ...
            invest_amount = st.number_input("投資金額 (税込)", 0.0, step=100000.0,
                                            format="%.0f", key=f"add_inv_amount_{i}")
            depr_years = st.number_input("耐用年数", 1, 50, 15, 1, key=f"add_inv_dep_{i}")

            loan_amt = st.number_input("借入金額 (任意)", 0.0, step=100000.0,
                                       format="%.0f", key=f"add_inv_loan_{i}")
            loan_years = st.number_input("借入期間（年）", 0, 50, 0, 1,
                                         key=f"add_inv_loany_{i}")
            loan_rate = st.number_input("借入金利（%）", 0.0, 50.0, 0.0,
                                        key=f"add_inv_loanr_{i}") / 100
            ...
            if invest_amount > 0:
                investments.append(
                    AdditionalInvestmentParams(
                        ...
                        loan_interest_rate=float(loan_rate),
                    )
                )
...
# ----------------------------------------------------------------------
# 3. サイドバー入力（SimulationParams 20項目に完全対応）
# ----------------------------------------------------------------------
def setup_sidebar() -> SimulationParams:
    ...
    price_bld = st.sidebar.number_input("建物価格（税込）", 0.0, value=50_000_000.0, format=CURRENCY)
    price_land = st.sidebar.number_input("土地価格", 0.0, value=30_000_000.0, format=CURRENCY)
    brokerage = st.sidebar.number_input("仲介手数料（税込）", 0.0, value=3_300_000.0, format=CURRENCY)
    ...
    loan_amt = st.sidebar.number_input("初期借入金額", 0.0, value=70_000_000.0, format=CURRENCY)
    loan_yrs = st.sidebar.number_input("返済期間（年）", 1, 50, 30)
    loan_rate = st.sidebar.number_input("借入金利（%）", 0.0, 50.0, 2.5) / 100

    initial_loan = (
        LoanParams(amount=float(loan_amt), interest_rate=float(loan_rate), years=int(loan_yrs))
        if loan_amt > 0
        else None
    )
    ...
    rent_income = st.sidebar.number_input("年間家賃収入（税込）", 0.0, value=3_600_000.0, format=CURRENCY)
    mgmt_fee = st.sidebar.number_input("年間管理費（税込）", 0.0, value=1_200_000.0, format=CURRENCY)
    repair_cost = st.sidebar.number_input("年間修繕費（税込）", 0.0, value=300_000.0, format=CURRENCY)
    insurance_cost = st.sidebar.number_input("年間保険料", 0.0, value=60_000.0, format=CURRENCY)
    other_mgmt_fee = st.sidebar.number_input("その他管理費", 0.0, value=0.0, format=CURRENCY)

    # CAP レート（目標）
    target_cap_rate = st.sidebar.number_input("ターゲットCAPレート（%）", 0.0, 50.0, 5.0) / 100

    mgmt_rate = st.sidebar.number_input("管理委託料率（%）", 0.0, 50.0, 0.0) / 100

    # 固定資産税
    tax_land = st.sidebar.number_input("固定資産税（土地）", 0.0, value=150_000.0, format=CURRENCY)
    tax_bld = st.sidebar.number_input("固定資産税（建物）", 0.0, value=150_000.0, format=CURRENCY)
    ...
    vat_rate = st.sidebar.number_input("消費税率（%）", 0.0, 50.0, 10.0) / 100
    non_tax_ratio = st.sidebar.number_input("非課税割合（0-1.0）", 0.0, 1.0, 0.0, 0.05)

    overdraft_rate = st.sidebar.number_input("短期借入利率（%）", 0.0, 30.0, 3.0) / 100
    discount_rate = st.sidebar.number_input("割引率（DCF用）（%）", 0.0, 30.0, 5.0) / 100
    ...
    sell_price = st.sidebar.number_input("売却価格", 0.0, value=0.0, format=CURRENCY)
    exit_tax_rate = st.sidebar.number_input("売却益税率（%）", 1, 60, 30) / 100

    exit_params = ExitParams(
        exit_year=exit_y,
        selling_price=float(sell_price),
        selling_cost=0.0,
        income_tax_rate=float(exit_tax_rate),
    )
    ...
    # ===============================
    # 20項目 + exit + investments + start_date の正式構成
    # ===============================
    return SimulationParams(
        ...
        annual_rent_income_incl=float(rent_income),
        annual_management_fee_initial=float(mgmt_fee),
        repair_cost_annual=float(repair_cost),
        insurance_cost_annual=float(insurance_cost),
        ...
        non_taxable_proportion=float(non_tax_ratio),
        overdraft_interest_rate=float(overdraft_rate),
        cf_discount_rate=float(discount_rate),
...
def economic_detective_report(fs_data: dict, params: SimulationParams, ledger_df: pd.DataFrame):
    ...
    roi = (
        (final_cash - params.initial_equity) / params.initial_equity
...
def main():
    ...
    st.caption("Amelia V20（SimulationParams 20項目統合版）")
    ...
    c1.markdown(
        f"""
        <div class="bkw-card">
            <div class="bkw-label">建物価格（税込）</div>
            <div class="bkw-value">{params.property_price_building:,.0f} 円</div>
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    c2.markdown(
        f"""
        <div class="bkw-card">
            <div class="bkw-label">保有年数</div>
            <div class="bkw-value">{params.holding_years} 年</div>
        </div>
        """,
        unsafe_allow_html=True
    )
